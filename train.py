...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    #viewpoint_stack = [viewpoint_stack[104]]
    #temp_list = copy.deepcopy(viewpoint_stack)
    #print(f"overfitting to {viewpoint_stack[0].frame_name}")
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if flag>0:
            for name, param in gaussians._deformation.named_parameters():
                if 'deform' in name and 'aabb' not in name and 'opacity' not in name and 'shs' not in name:
                    print(f"{name} {torch.isnan(torch.autograd.grad(loss,param, retain_graph=True)[0]).any()}")
            time_=torch.tensor(viewpoint_cam.time).to(gaussians._xyz.device).repeat(gaussians._xyz.shape[0],1)
            grid_feature = gaussians._deformation.deformation_net.grid(gaussians._xyz[:,:3], time_[:,:1])
            hidden = torch.cat([grid_feature],-1)
            mask = torch.ones_like(gaussians._opacity[:,0]).unsqueeze(-1)
            hidden_2 = gaussians._deformation.deformation_net.feature_out(hidden)
            ds = gaussians._deformation.deformation_net.scales_deform(hidden_2)
            scales = torch.zeros_like(gaussians._scaling[:,:3])
            scales = gaussians._scaling[:,:3]*mask + ds
            # nan position across number of gaussians
            pdb.set_trace()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[14000, 28000])
