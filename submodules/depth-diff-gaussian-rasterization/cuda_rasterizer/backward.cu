	...
	float* dL_dcov)
{
    ...
    const float af = a - h_var;
    const float cf = c - h_var;
    const float bf = b;
    const float al = a;
    const float cl = c;
    const float denom_f = d_inside_root / sq(det_cov_plus_h_cov);
    const float dL_daf = (cf * det_cov_plus_h_cov - det_cov * cl) * denom_f;
    const float dL_dcf = (af * det_cov_plus_h_cov - det_cov * al) * denom_f;
    const float dL_dbf = (-2*bf*(det_cov_plus_h_cov -det_cov) )* denom_f;
    dL_da = dL_daf;
    dL_dc = dL_dcf;
    dL_db = dL_dbf;


	float denom = det_cov_plus_h_cov;
	//float denom = a * c - b * b;
	//float dL_da = 0, dL_db = 0, dL_dc = 0;
	//#-------------------------------------------------------------------------#
